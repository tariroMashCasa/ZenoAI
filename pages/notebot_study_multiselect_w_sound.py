...
# Path to your trained model directory (adjust accordingly)
model_path = "/Users/tariromashongamhende/Documents/Documents - Tariro’s MacBook Pro/ml_projects/project_siren/tts_model_vanilla/best_model_140645.pth"
...
class GPT4o_test_performance_reviewer:
    ...
    def send_command(self, command):
        ...
        response = client.chat.completions.create(
                                                ...
                                                max_tokens=800,  # Adjust based on the length of the completion
                                                temperature=0.1  # Controls randomness (lower = more focused)
                                                )
        response = response.choices[0].message.content
        self.history.append((command, response))
            ...
            if len(uploaded_detailed_notes_pdf)>0:
                ...
                for value in range(len(list_of_questions)):
                    ...
                    with st.expander("Read out the question"):
                        ...
                        # this is the new audio speech using the custom trained text to speech model
                        with tempfile.TemporaryDirectory() as temp_dir:
                            # Generate speech
                            output_wav_path = f"question_{value}.wav"
                            text_to_speak = f"{list_of_questions[value].replace('Q','Question ')}"

                            valid_tts_generation = False
                            while valid_tts_generation==False:
                                try:
                                    tts.tts_to_file(text=text_to_speak, file_path=output_wav_path)
                                    valid_tts_generation = True
                                    pass
                                except Exception as e:
                                    continue
                                # tts.tts_to_file(text=text_to_speak, file_path=output_wav_path)
                            ...
                            temp_file_path = os.path.join(temp_dir, output_wav_path)
                    ...
                    # else:
                        ...
                        with st.expander("Read out the possible answers"):
                            ...
                            with tempfile.TemporaryDirectory() as temp_dir:
                                # Generate speech
                                output_wav_path = "possible_answers.wav"
                                text_to_speak = ''.join(list_of_answer_options)
                ...
                if False not in st.session_state.submitted:
                    if st.button("Show all submitted answers"):
                        ...
                        with tempfile.TemporaryDirectory() as temp_dir:
                            # Generate speech
                            output_wav_path = "test_performance_summary.wav"
                            text_to_speak = overall_summary
